...
class RateLimitMiddleware(BaseMiddleware):
    """Ограничивает частоту скачиваний — только для текстовых сообщений с Twitch-ссылками"""
    ...
    async def __call__(
        ...
        # проверяем что текст похож на ссылку Twitch
        from bot.utils.helpers import is_twitch_url
        if not is_twitch_url(event.text.strip()):
            return await handler(event, data)
